...
class User {
  ...
  // SBO Attendance Functions
  function getAllTribes()
  {
    include "connection.php";

    $sql = "SELECT tribe_id, tribe_name FROM tbltribe ORDER BY tribe_name";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
  ...
  function getStudentsInTribe($json)
  {
    ...
    // Get students in the specified tribe
    ...
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':tribeId', $tribeId);
    $stmt->execute();
